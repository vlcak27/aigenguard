"""Policy onboarding helpers for AigenGuard."""
...
AUDIT_POLICY_TOML = """# Audit AigenGuard policy for first reviews.
...
SAFE_POLICY_TOML = """# Safe AigenGuard policy for local pre-commit guard use.
...
STRICT_POLICY_TOML = """# Stricter AigenGuard policy.
...
def suggested_policy_toml(bom: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Suggested AigenGuard policy from current scan findings.",
