"""Local pre-commit guard support for AigenGuard."""
...
class ExistingHookError(ValueError):
    """Raised when a pre-existing non-AigenGuard hook would be changed implicitly."""
...
def run_guard(
    ...
    *,
    ...
    if not isinstance(policy_review, dict):
        print("aigenguard: guard requires an AigenGuard TOML policy file.", file=err)
        return 1
    ...
    if guard_mode == "confirm":
        ...
        if answer is None:
            print("aigenguard confirm mode requires an interactive terminal.", file=out)
            print(
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            'if [ -z "$repo_root" ]; then',
            '  echo "AigenGuard policy guard could not find repository root." >&2',
            "  exit 1",
            "fi",
            'cd "$repo_root" || exit 1',
            "",
